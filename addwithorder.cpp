...
bool reachibility_check(const Graph &g, int rows, int cols) {
                    ...
                    if (distance[local_out] == -1) {
                        ...
                        for (int i = 0; i < all_paths.size(); i++) {
                            ...
                            for (int j = 0; j < all_paths[i].size(); j++) {
                                auto [row, col, type] = get_node_info(all_paths[i][j], rows, cols);
                                cout << "(" << row << "," << col << "," << type << ")";
                                if (j != all_paths[i].size() - 1) cout << " -> ";
                            }
                            cout << "\n";
                        }
                        */
...
int main() {
    ...
    // Optional: Shuffle candidates for randomness
    // shuffle(candidates.begin(), candidates.end(), mt19937(123));
    ...
    for (auto &edge : candidates) {
        ...
        if (minimal_graph[u][v] == 1) continue;
        // Skip if adding the edge creates a cycle
        if (would_create_cycle(u, v, Paths)) {
            continue;
        }
        // Safely add the edge
        add_edge_and_update_paths(u, v, minimal_graph, Paths);
    }

    // 5) Validate results
    cout << "Total edges: " << count_edges(minimal_graph) << "\n";

    // Use Floyd-Warshall to check for cycles
    floyd_warshall_is_acyclic(minimal_graph);

    /* Check constraints
    if (check_constraints(minimal_graph)) {
    ...
    */
    ...
    bool reachable = reachibility_check(minimal_graph, rows, cols);
    if (reachable) {
